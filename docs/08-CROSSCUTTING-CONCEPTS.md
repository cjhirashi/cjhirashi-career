...
**Registro centralizado de fallas (`error_reports`, [ADR-018](./09-DECISIONS/018-error-reports-registry.md))**: además del logging a `stdout`, toda falla *inesperada* deja una fila consultable en la tabla `error_reports`. Es distinto de `audit_logs` (registro de negocio) y del logging de aplicación (efímero): su propósito es operativo — *¿qué está roto y qué falta por arreglar?*. Se escribe por un punto único (`services/error_reporting.py::report_error`, engine psycopg2 propio, nunca lanza, deduplica por `fingerprint` con contador `occurrences`) desde: los manejadores globales de la API (excepciones no controladas, `HTTPException` 5xx y 4xx salvo 401/404, validación), los `except` de fallos inesperados en servicios/schedulers/loop de Bedrock, el endpoint público `POST /system/error-report` (SPA Admin y Portfolio vía interceptor de axios + `window.onerror`; MCP server), y con `capture_errors(...)` en los schedulers in-process. El atributo `resolved` arranca en `false` (pendiente de revisión) y pasa a `true` cuando el problema se corrige. Consulta y gestión: Admin → *Settings → Reportes de Falla*, tool de Bedrock `error_report_settings`, y el agente `revisor-fallas`.
