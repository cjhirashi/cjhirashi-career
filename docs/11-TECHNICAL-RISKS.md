...
| R13 | Ruido y crecimiento de la tabla `error_reports` ([ADR-018](./09-DECISIONS/018-error-reports-registry.md)): errores repetitivos o escaneo de bots podrían inflarla; el endpoint público `POST /system/error-report` es superficie sin autenticación | Media | Media | Deduplicación por `fingerprint` con contador `occurrences` (un error en bucle es una fila); se omiten 401/404; constante `REPORT_CLIENT_ERRORS` para bajar el volumen de 4xx; rate-limit por IP + límite de tamaño de cuerpo en el endpoint público; `POST /settings/error-reports/purge-resolved` para purgar resueltos antiguos |
